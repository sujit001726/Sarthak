...
<head>
    ...
    <title>Sarthak | Search Companies</title>
        ...
        <div class="main-content-wrapper">
            ...
            <main class="p-6 lg:p-8 flex flex-col gap-8 w-full max-w-full">
                ...
                <div class="flex flex-col md:flex-row justify-between gap-6">
                    <div>
                        <h1 class="text-3xl font-black text-dark italic tracking-tighter uppercase border-l-8 border-primary pl-6">Company Network</h1>
                        <p class="text-sm font-bold text-gray-400 mt-2 pl-8">Discover and connect with other companies on Sarthak</p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-[2rem] p-6 border border-gray-100 shadow-sm flex flex-col md:flex-row gap-4 items-center">
                    <div class="relative w-full flex-1 group">
                        ...
                        <input type="text" id="companySearchInput" placeholder="Search companies by name..." class="w-full bg-surface border-none rounded-2xl py-4 pl-14 pr-6 text-sm font-bold focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all">
                    </div>
                    <button class="w-full md:w-auto bg-primary text-white px-10 py-4 rounded-2xl text-[0.8rem] font-black shadow-xl shadow-primary/20 hover:scale-[1.05] transition-all">Search Network</button>
                </div>
                    ...
                    <c:forEach var="company" items="${companies}">
                        <div class="company-card bg-white border border-gray-100 rounded-[2rem] p-6 flex flex-col gap-4 relative overflow-hidden group">
                            ...
                            <div class="mt-auto flex gap-2 pt-4">
                                <a href="${pageContext.request.contextPath}/messages?userId=${company.id}" class="flex-1 text-center bg-primary text-white py-2.5 rounded-xl text-xs font-black hover:bg-dark transition-all">
                                    <i class="fa-regular fa-paper-plane mr-2"></i> Message
                                </a>
                            </div>
                        </div>
                    </c:forEach>
                    <c:if test="${empty companies}">
                        <div class="col-span-full p-10 text-center text-gray-400 italic text-sm">No companies found on the network.</div>
                    </c:if>
                </div>
            </main>
        </div>
