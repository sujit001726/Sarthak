                    ...
                    <c:forEach var="talent" items="${talents}">
                        <div class="talent-card bg-white border border-gray-100 rounded-[2rem] p-6 flex flex-col gap-4 relative overflow-hidden group">
                            <div class="flex items-center gap-4">
                                <div class="w-14 h-14 bg-blue-100 rounded-[1.2rem] flex items-center justify-center overflow-hidden shadow-lg shrink-0">
                                    <img src="${pageContext.request.contextPath}/image?userId=${talent.id}&type=profile"
                                        onerror="this.outerHTML='<div class=\'w-full h-full flex items-center justify-center text-blue-600 text-xl font-black\'>${talent.fullName.substring(0,1).toUpperCase()}</div>'"
                                        class="w-full h-full object-cover">
                                </div>
                                <div class="min-w-0">
                                    <h3 class="text-lg font-black text-dark leading-none truncate">${talent.fullName}</h3>
                                    <p class="text-[0.65rem] font-bold text-gray-400 mt-1 uppercase truncate">${talent.email}</p>
                                </div>
                            </div>
                            <div class="flex flex-wrap gap-2 mt-2">
                                <span class="bg-surface px-2 py-1 rounded-md text-[0.55rem] font-black text-secondary uppercase">Job Seeker</span>
                            </div>
                            <p class="text-xs text-gray-500 font-medium line-clamp-2">Registered talent on the Sarthak platform.</p>
                            <div class="mt-auto flex gap-2 pt-4">
                                <a href="${pageContext.request.contextPath}/profile?id=${talent.id}" class="flex-1 text-center bg-surface text-primary py-2.5 rounded-xl text-xs font-black hover:bg-primary hover:text-white transition-all">View Profile</a>
                                <a href="${pageContext.request.contextPath}/messages?userId=${talent.id}" class="w-10 flex items-center justify-center bg-surface text-primary rounded-xl hover:bg-primary hover:text-white transition-all"><i class="fa-regular fa-envelope"></i></a>
                            </div>
                        </div>
                    </c:forEach>
                    <c:if test="${empty talents}">
                        <div class="col-span-full p-10 text-center text-gray-400 italic text-sm">No talent profiles found.</div>
                    </c:if>
                </div>
            </main>
