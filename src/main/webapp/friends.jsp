        ...
        <head>
            ...
            <style>
                ...
                .friend-card:hover {
                    ...
                    box-shadow: 0 10px 25px -5px rgba(29, 62, 53, 0.1);
                }
            </style>
        </head>
                    ...
                    <%@ include file="/includes/header.jsp" %>
                        ...
                        <main class="p-8 md:p-12 w-full max-w-7xl mx-auto">
                            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                                <div>
                                    <h1
                                        class="text-3xl font-black text-dark border-l-8 border-primary pl-6 uppercase tracking-tighter italic">
                                        My Network</h1>
                                    <p class="text-gray-400 font-bold mt-2 ml-8">Connect with professionals and grow
                                        your circle.</p>
                                </div>
                                <div class="flex items-center gap-4 relative">
                                    <div class="relative">
                                        ...
                                        <div id="search-results"
                                            class="absolute top-full left-0 right-0 mt-4 bg-white rounded-2xl shadow-2xl border border-gray-100 hidden z-50 overflow-hidden">
                                            <div class="p-4 border-b border-gray-50 bg-surface/50">
                                                <p
                                                    class="text-[0.6rem] font-black text-gray-400 uppercase tracking-widest">
                                                    Search Results</p>
                                            </div>
                                    ...
                                    <button
                                        class="bg-primary text-white px-8 py-3.5 rounded-2xl font-black text-sm shadow-xl shadow-primary/20 hover:scale-105 transition-all">Add
                                        Friend</button>
                                </div>
                            </div>
                            ...
                            <c:if test="${not empty pendingRequests}">
                                <div class="mb-12">
                                    <div class="flex items-center gap-3 mb-6">
                                        <h2 class="text-lg font-black text-dark uppercase tracking-tight">Friend
                                            Requests</h2>
                                        <span
                                            class="bg-red-500 text-white text-[0.6rem] font-bold px-2 py-0.5 rounded-full">${pendingRequests.size()}
                                            New</span>
                                    </div>
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                        <c:forEach items="${pendingRequests}" var="request">
                                            <div id="request-card-${request.id}"
                                                class="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm flex items-center gap-4 transition-all hover:shadow-md">
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <p
                                                        class="text-[0.6rem] font-bold text-gray-400 uppercase tracking-widest truncate">
                                                        ${request.role}</p>
                                                    <div class="flex items-center gap-2 mt-3">
                                                        <button onclick="handleRequestAction(${request.id}, 'accept')"
                                                            class="flex-1 py-2 bg-primary text-white rounded-lg text-[0.65rem] font-black hover:bg-secondary transition-all">Confirm</button>
                                                        <button onclick="handleRequestAction(${request.id}, 'reject')"
                                                            class="flex-1 py-2 bg-gray-100 text-dark rounded-lg text-[0.65rem] font-black hover:bg-gray-200 transition-all">Reject</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </c:forEach>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                <c:forEach items="${friends}" var="friend">
                                    <div
                                        class="friend-card bg-white rounded-3xl p-6 border border-gray-50 shadow-sm flex flex-col items-center text-center">
                                        <div class="relative mb-4">
                                            ...
                                            <div
                                                class="absolute bottom-1 right-1 w-5 h-5 border-4 border-white rounded-full ${friend.online ? 'bg-accent' : 'bg-gray-400'}">
                                            </div>
                                        </div>

                                        <h3 class="text-lg font-black text-dark mb-1">${friend.name}</h3>
                                        <p
                                            class="text-[0.65rem] font-bold text-gray-400 uppercase tracking-widest mb-4">
                                            ${friend.role}</p>

                                        <div class="flex items-center gap-2 mb-6">
                                            <span
                                                class="px-3 py-1 bg-surface text-[0.6rem] font-black text-primary rounded-lg border border-gray-100 uppercase">Nepal</span>
                                            <span
                                                class="px-3 py-1 bg-surface text-[0.6rem] font-black text-primary rounded-lg border border-gray-100 uppercase">Tech</span>
                                        </div>

                                        <div class="grid grid-cols-2 gap-3 w-full">
                                            <a href="${pageContext.request.contextPath}/messages?userId=${friend.id}"
                                                class="flex items-center justify-center gap-2 py-3 bg-primary text-white rounded-xl text-xs font-black shadow-lg shadow-primary/10 hover:bg-secondary transition-all">
                                                <i class="fa-solid fa-message-dots"></i> Message
                                            </a>
                                            <a href="${pageContext.request.contextPath}/profile?userId=${friend.id}"
                                                class="flex items-center justify-center gap-2 py-3 bg-white border border-gray-100 text-dark rounded-xl text-xs font-black hover:bg-gray-50 transition-all">
                                ...
                                <c:if test="${empty friends}">
                                    <div class="col-span-full py-20 text-center">
                                        <div
                                            class="w-24 h-24 bg-white rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm border border-gray-50">
                                        ...
                                        <p class="text-gray-400 font-bold mt-2">Start searching for professionals to
