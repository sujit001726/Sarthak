...
<body class="text-gray-900 bg-surface">
                            ...
                            <div class="kyc-card">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                    ...
                                    <div class="bg-surface border border-gray-100 rounded-2xl p-5">
                                        ...
                                        <button type="button" onclick="openCvUploadModal()" class="text-xs font-black text-primary mt-3 hover:underline">Update Resume</button>
                                    </div>
                                </div>
                            ...
                            <form id="profileForm" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="md:col-span-2 space-y-2">
                                    ...
                                    <div class="flex items-center justify-between p-4 bg-primary/5 border border-primary/10 rounded-xl">
                                        ...
                                        <c:if test="${isOwnProfile}">
                                            <button type="button" onclick="openCvUploadModal()" class="text-primary font-black text-xs hover:underline">Replace</button>
                    ...
                    <div class="flex flex-col gap-8">
                        ...
                        <div class="kyc-card">
                            <div class="flex items-center justify-between mb-6">
                                ...
                                <a href="${pageContext.request.contextPath}/friends" class="text-[0.6rem] font-black text-primary hover:underline">View All</a>
                            </div>
                            <div class="grid grid-cols-2 gap-4">
                                <c:forEach items="${friends}" var="friend">
                                    <a href="${pageContext.request.contextPath}/profile?userId=${friend.id}" 
                                        class="flex flex-col items-center gap-2 p-3 bg-surface rounded-2xl group cursor-pointer hover:bg-white hover:shadow-md transition-all">
                                        <img src="${pageContext.request.contextPath}/image?userId=${friend.id}&type=profile" class="w-12 h-12 rounded-full border-2 border-white shadow-sm transition-transform group-hover:scale-105" onerror="this.src='https://ui-avatars.com/api/?name=${friend.name}&background=1D3E35&color=fff'">
                                        <p class="text-[0.7rem] font-bold text-dark text-center truncate w-full group-hover:text-primary transition-colors">${friend.name}</p>
                                    </a>
                                </c:forEach>
    ...
    <script>
        ...
        document.getElementById('resume-upload')?.addEventListener('change', function() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    const seekerHubResumeName = document.getElementById('seeker-hub-resume-name');
                    if (nameEl) nameEl.textContent = data.fileName;
                    if (uploadedEl) uploadedEl.textContent = 'Uploaded just now';
                    if (seekerHubResumeName) seekerHubResumeName.textContent = data.fileName;
        ...
        window.addEventListener('DOMContentLoaded', () => {
            ...
            } else if (params.get('updateCv') === 'true') {
                openCvUploadModal();
            }
        });
    ...
    <div id="cvUploadModal"
        ...
        <div class="bg-white rounded-[2rem] p-8 w-full max-w-xl shadow-2xl transform scale-95 transition-transform duration-300"
             id="cvUploadModalContent">
            <div class="flex justify-between items-start mb-6 border-b border-gray-100 pb-4">
                <div>
                    <h2 class="text-2xl font-black text-dark tracking-tighter uppercase italic text-primary">Update CV / Resume</h2>
                    <p class="text-xs font-bold text-gray-400 mt-1">Upload your latest PDF or Word CV to stand out to employers</p>
                </div>
                <button onclick="closeCvUploadModal()"
            ...
            <!-- Current Resume Card -->
            <div class="bg-surface border border-gray-100 rounded-2xl p-5 mb-6 flex items-center gap-4">
                <div class="w-12 h-12 bg-red-50 rounded-xl flex items-center justify-center shrink-0 text-red-500 text-xl">
                    <i class="fa-solid fa-file-pdf"></i>
                </div>
                <div class="min-w-0 flex-1">
                    <p class="text-xs font-black text-gray-400 uppercase tracking-wider">Active Resume</p>
                    <h4 id="modal-resume-filename" class="text-sm font-black text-dark truncate mt-0.5">
                        <c:choose>
                            <c:when test="${not empty resumeFileName}">${resumeFileName}</c:when>
                            <c:otherwise>No resume uploaded yet</c:otherwise>
                        </c:choose>
                    </h4>
                    <p id="modal-resume-date" class="text-[0.65rem] font-bold text-gray-400 mt-0.5">
                        <c:choose>
                            <c:when test="${not empty resumeUploadedAt}">Uploaded on ${resumeUploadedAt}</c:when>
                            <c:otherwise>Stand out by adding your credentials</c:otherwise>
                        </c:choose>
                    </p>
                </div>
            </div>
            ...
            <!-- Drag & Drop Zone -->
            <div id="cv-dropzone" 
                 class="border-2 border-dashed border-gray-200 rounded-3xl p-8 text-center bg-surface hover:border-primary/50 transition-all cursor-pointer group flex flex-col items-center justify-center">
                <input type="file" id="modal-resume-upload-input" class="hidden" accept="application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,.pdf,.docx">
                <div class="w-16 h-16 bg-primary/5 text-primary rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                    <i class="fa-solid fa-cloud-arrow-up text-2xl"></i>
                </div>
                <h3 class="text-xs font-black text-dark uppercase tracking-wider">Drag & Drop Resume Here</h3>
                <p class="text-[0.65rem] font-bold text-gray-400 mt-1">Or click to browse files</p>
                <p class="text-[0.6rem] text-gray-400 mt-4 leading-relaxed bg-white px-3 py-1 rounded-full border border-gray-100">Supports PDF and DOCX formats (Max 5MB)</p>
            </div>

            <!-- Footer actions -->
            <div class="mt-6 flex justify-end gap-3">
                <button onclick="closeCvUploadModal()" class="px-5 py-3 bg-gray-50 text-gray-500 rounded-xl text-xs font-black hover:bg-gray-100 transition-colors">
                    Cancel
                </button>
                <button onclick="document.getElementById('modal-resume-upload-input').click()" class="px-5 py-3 bg-primary text-white rounded-xl text-xs font-black shadow-lg shadow-primary/20 hover:scale-105 transition-all">
                    Select File
                </button>
            </div>
        </div>
    ...
    <script>
        function openCvUploadModal() {
            ...
            const content = document.getElementById('cvUploadModalContent');
            if (!modal) return;
        ...
        const modalInput = document.getElementById('modal-resume-upload-input');

        if (dropzone && modalInput) {
            dropzone.addEventListener('click', () => modalInput.click());
            ...
            dropzone.addEventListener('drop', (e) => {
                ...
                if (e.dataTransfer.files.length > 0) {
                    handleModalResumeFile(e.dataTransfer.files[0]);
                }
            });

            modalInput.addEventListener('change', function() {
                if (this.files.length > 0) {
                    handleModalResumeFile(this.files[0]);
                }
            });
        }

        function handleModalResumeFile(file) {
            if (!file) return;

            const dropzoneText = dropzone.querySelector('h3');
            if (!dropzoneText) return;
            const originalH3 = dropzoneText.textContent;

            dropzoneText.textContent = "Uploading " + file.name + "...";
            dropzoneText.classList.add('text-primary');
            ...
            .then(data => {
                if (data.success) {
                    const modalFilename = document.getElementById('modal-resume-filename');
                    const modalDate = document.getElementById('modal-resume-date');
                    const seekerHubResumeName = document.getElementById('seeker-hub-resume-name');
                    if (modalFilename) modalFilename.textContent = data.fileName;
                    if (modalDate) modalDate.textContent = 'Uploaded just now';
                    if (seekerHubResumeName) seekerHubResumeName.textContent = data.fileName;

                    const outerFilename1 = document.getElementById('resume-file-name');
                    const outerDate1 = document.getElementById('resume-uploaded-at');
                    if (outerFilename1) outerFilename1.textContent = data.fileName;
                    if (outerDate1) outerDate1.textContent = 'Uploaded just now';

                    let outerLink = document.getElementById('resume-link');
                    if (!outerLink && outerFilename1) {
                        outerLink = document.createElement('a');
                        outerLink.id = 'resume-link';
                        outerLink.target = '_blank';
                        outerLink.className = 'text-sm font-bold text-dark hover:text-primary hover:underline';
                        outerFilename1.parentNode.replaceChild(outerLink, outerFilename1);
                        outerLink.appendChild(outerFilename1);
                    }
                    if (outerLink) outerLink.href = data.downloadUrl;

                    const sidebarResumeText = document.querySelector('.truncate');
                    if (sidebarResumeText && sidebarResumeText.textContent.includes('Not uploaded')) {
                        sidebarResumeText.textContent = data.fileName;
                    }
                    ...
                    showToast("Error", data.message || 'Failed to upload resume.');
                }
                dropzoneText.textContent = originalH3;
                dropzoneText.classList.remove('text-primary');
            })
            .catch(error => {
                console.error('Error uploading resume:', error);
